...
export default (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("iniciar", async (codigo) => {
      ...
      if (juegos.nombres[codigo].length === 1) {
        ...
        const { cartasJugador, number } = repartirCartas(
          juegos.id[codigo],
          cartasss,
          numero
        );
        ...
        juegos.id[codigo].map((jugadorr) => {
          ...
          io.to(jugadorr).emit("cartass", cartaporJugador, numeracioncard);
        });
      }
    });
  ...
  function repartirCartas(jugadores, cartaxxx, numero) {
